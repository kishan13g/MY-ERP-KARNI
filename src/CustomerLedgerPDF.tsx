...
export const CustomerLedgerPDF: React.FC<Props> = ({ data: propData, onClose }) => {
    ...
    <>
      ...
      <div id="ledger-print" className="fixed top-16 left-1/2 -translate-x-1/2 z-[9999] w-[95%] max-w-[900px] bg-white shadow-2xl rounded-lg overflow-hidden flex flex-col p-8" onClick={e => e.stopPropagation()}>
        <div className="text-center border-b-2 border-slate-800 pb-4 mb-6">
          ...
          <p className="text-sm text-slate-600 uppercase tracking-widest">Account Ledger</p>
        </div>

        <div className="grid grid-cols-2 gap-8 mb-8">
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Account Name</label>
        ...
        <table className="w-full border-collapse text-xs">
          ...
          <tfoot>
            <tr className="bg-slate-50 font-bold border-t-2 border-slate-800">
              ...
              <td className="p-3 text-right text-emerald-700">{closingBalance.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
            </tr>
          </tfoot>
        </table>
